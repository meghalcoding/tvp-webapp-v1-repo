...
function accountForm(screen, account) {
  ...
  mount.innerHTML = `<div class="modal-backdrop"><form class="modal-card" id="account-form"><button type="button" class="modal-close" aria-label="Close">×</button><h2>${account ? "Edit account" : "New account"}</h2>
    ...
    <div class="field"><label>Account type</label><select name="type"><option value="cash" ${selected("cash")}>Cash</option><option value="bank" ${selected("bank")}>Bank</option><option value="collection_account" ${selected("collection_account")}>Collection account (UPI)</option><option value="payable" ${selected("payable")}>Payable / Due</option></select></div>
