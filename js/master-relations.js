...
export async function loadMasterRelations() {
  const [channels, purchaseCategories, expenseCategories, items, purchaseMap, expenseMap, suppliers, supplierPurchase, supplierExpense] = await Promise.all([
    ...
    supabase.from('purchase_category_items').select('purchase_category_id,item_id,active').eq('active',true),
    supabase.from('expense_category_items').select('expense_category_id,item_id,active').eq('active',true),
