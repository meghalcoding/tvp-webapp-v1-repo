...
export async function renderExpensesScreen(screen, user) {
  ...
  const expenses = await fetchTransactionPage({ type: "expense", select: "id,txn_date,amount,description,created_at,expense_details(category_id,paid_from_account_id),expense_items(id)", limit: HISTORY_INITIAL_LIMIT });
  ...
  const expenseForm = screen.querySelector("#expense-form");
  ...
  screen.querySelector("#expense-form")?.addEventListener("submit", async (event) => {
    ...
    const items=[...(rows?.querySelectorAll(".line-item-row")||[])].map((row)=>Object.fromEntries([...row.querySelectorAll("[data-item]")].map((input)=>[input.dataset.item,input.value.trim()])));
    ...
    const payload={p_client_uuid:crypto.randomUUID(),p_category_id:form.get("category_id"),p_paid_from_account_id:form.get("paid_from_account_id") || null,p_amount:Number(form.get("amount")),p_txn_date:form.get("txn_date"),p_description:form.get("description").trim()||null,p_items:items};
