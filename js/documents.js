...
export async function openDocumentModal({ screen, transactionId = null, supplierId = null, user, onDone }) {
  ...
  async function renderExisting() {
    ...
    existing.querySelectorAll(".document-open").forEach((b) => b.addEventListener("click", async () => { try { b.disabled = true; const url = await signedUrl(b.dataset.path); window.open(url, "_blank", "noopener"); } catch (e) { alert(e.message); } finally { b.disabled = false; } }));
    existing.querySelectorAll(".document-unlink").forEach((b) => b.addEventListener("click", async () => { try { b.disabled = true; await unlinkDocument(b.dataset.id, transactionId); await renderExisting(); onDone?.(); } catch (e) { alert(e.message); b.disabled = false; } }));
  }
...
export async function openLinkExistingDocumentModal({ screen, documentId, onDone }) {
  let pickerTransactions = [];
  try { pickerTransactions = await loadTransactionsForPicker(); } catch (error) { alert(error.message); return; }
  const mount = document.createElement("div");
  mount.className = "document-modal-mount";
...
export async function renderDocumentsScreen(screen, user) {
  ...
  async function load() {
    ...
    body.querySelectorAll(".document-open-library").forEach((b) => b.addEventListener("click", async () => { try { const url = await signedUrl(b.dataset.path); window.open(url, "_blank", "noopener"); } catch (e) { alert(e.message); } }));
    body.querySelectorAll(".document-link-library").forEach((b) => b.addEventListener("click", async () => { try { await openLinkExistingDocumentModal({ screen, documentId: b.dataset.id, onDone: load }); } catch (e) { alert(e.message); } }));
  }
